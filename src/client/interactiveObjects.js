...
class InteractiveObject {
    constructor(entityId, text, useDirection, onUse, timeBetweenUses, textOffset = [0, 0, 0.1]) {
...
export default function initInteractiveObjects() {
    ...
    setTick(() => {
        ...
        let nearbyObject = interactiveObjects.filter((obj) => {
            return gameTimer > obj.lastUsedTime + obj.timeBetweenUses;
        }).map((obj) => {
                    ...
                    if (nearbyObject.distance <= MAX_USE_DISTANCE) {
                        ...
                        nearbyObject.onUse(nearbyObject)
                                    ...
                                    .finally(() => {
                                        ...
                                        const obj = interactiveObjects.find((obj) => obj.entity === nearbyObject.entity);
                                        if (obj) {
                                            // Object in original array, update the next use timer
                                            obj.lastUsedTime = GetGameTimer() + obj.timeBetweenUses;
                                        }
                                    });
                    }
                }
    ...
    addEventListener('interactions:register', (entity, text, direction, onUse, timeBetweenUses, textOffset) => {
        interactiveObjects = interactiveObjects.filter((obj) => obj.entity !== entity);

        interactiveObjects.push(new InteractiveObject(entity, text, direction, onUse, timeBetweenUses, textOffset))
    });
        ...
        for (let entity of objects) {
            ...
            if (ObjectHandlers.hasOwnProperty(modelHash.toString()) && !interactiveObjects.find((obj) => obj.entity === entity)) {
                ...
                const args = {
                    ...
                    timeBetweenUses: objData.hasOwnProperty('timeBetweenUses') ? objData.timeBetweenUses : 5000,
                };

                emit('interactions:register', args.entity, args.text, args.useDirection, args.onUse, args.timeBetweenUses, args.textOffset);
            }
        }
    }, 5000));
}
